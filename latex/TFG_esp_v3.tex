...
\end{equation}
donde $g_k = \nabla f(x_k)$ y $G_k = \nabla^2f(x_k)$.
Este modelo cuadrático es utilizado en los llamados métodos de Newton para elegir la dirección (son del tipo búsqueda de línea).
Los métodos de región de confianza aprovechan la eficacia de usar este modelo, pero evitando algunas limitaciones y obteniendo la convergencia global.
...
\end{aligned}
\end{equation}
Como vemos, en el modelo se escribe $B_k$ en lugar de $G_k$, pues no siempre se usa esta última. Debido al coste computacional, a veces se prefiere aproximar de alguna manera más o menos eficiente, e incluso puede ser aceptable tomar la matriz $0$.

También se puede variar qué norma define la región de confianza, cambiando así la forma de esta y ofreciendo distintos resultados, aunque generalmente se utiliza la bola definida por $\Vert p \Vert_2 \leq \Delta$.
