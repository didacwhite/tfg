...
\begin{algorithm}[Región de confianza] \leavevmode
	...
	\end{steps}
\end{algorithm}
En el paso 5, el primer caso se dice que resulta en una iteración no exitosa, mientras que el segundo caso marcará
una iteración exitosa y el tercero una muy exitosa.
Añadir que $\bar \Delta$ es el máximo radio de la región para toda iteración y que únicamente se aumenta el radio
en el caso de que $\norm{p_k} = \Delta_k$,
...
Respecto a la convergencia de este algoritmo, veremos únicamente un resultado relevante para el estudio realizado en el
tercer capítulo. Para ello, veamos primero una serie de condiciones que se suponen ciertas para los resultados
relativos a este algoritmo.

Suponemos que $\norm{B_k}$ es uniformemente acotada para cada $k$
y que el conjunto de nivel $\{x\,|\,f(x)\leq f(x_0)\}$ es acotado, con $f: \mathbb{R}^n \to \mathbb{R}$ continuamente
...
Bajo estas condiciones comunes, si el algoritmo (\ref{alg:tr}) tiene las suficientes iteraciones exitosas,
...
\begin{theorem}
	...
	Entonces,
	\begin{equation}
		\lim_{k\rightarrow \infty} \nabla f(x_k) = \lim_{k\rightarrow \infty} J_k^Tr_k = 0.
	\end{equation}
\end{theorem}
...
\begin{equation}
	D_kp(0) \equiv \lim_{\lambda \rightarrow 0^+} D_kp(\lambda) =
...
(\ref{eq:more}). En este caso, Moré elige la factorización QR. Si bien es cierto que la resolución
...
y $D_k^TD_k$ puede no ser adecuada.
...
evitar errores numéricos por desbordamiento, multiplicamos en ambos lados de las ecuaciones normales (\ref{normalmore})
...
\subsection{Escalado}
Ya que el propósito de la matriz $D_k$ en el algoritmo de Levenberg-Marquardt es tener en cuenta
el escalado del problema, elegimos
\begin{equation}
	D_k = diag(d_1^{(k)}, \ldots, d_n^{(k)}),
\end{equation}
con $d_i^{(0)} = \norm{\partial_ir(x_0)}$, y
$d_i^{(k)}=\max \{d_i^{(k-1)}, \norm{\partial_ir(x_k)}\}$, con $k \geq 1$.
