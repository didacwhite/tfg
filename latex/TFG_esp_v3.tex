...
Podemos realizar otras configuraciones para que se nos muestren más información, se muestra un ejemplo en la figura \ref{fig:lsqnonlin-iter}. En este ejemplo se prueba con la primera función residuo mostrada por Moré en \cite[Apartado 8]{More1978-at}:
...
Según la documentación esta configuración utiliza el algoritmo de Levenberg-Marquardt referenciando también a Moré, así que se puede decir que es una implementación del algoritmo de Moré. Pero con el objetivo de entender mejor el funcionamiento del algoritmo y ponerlo en práctica, se ha implementado una versión simplificada de la descrita en el capítulo anterior con el propósito de ponerla a prueba al lado del comando \code{lsqnonlin}. De aquí en adelante cuando mostremos el comando \code{myLevMar} nos referimos a la implementación mostrada en el Anexo \ref{chap:impl-simpl}.
...
\end{lstlisting}
La función \code{theta()} definida según (\ref{eq:theta}):
...
\chapter{Implementación versión simplificada}\label{chap:impl-simpl}
