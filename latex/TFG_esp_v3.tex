...
Veamos cómo se enfoca este sistema de ecuaciones, conocidas como \textit{ecuaciones normales} de (\ref{eq:flin}). El primer algoritmo que se plantea es a partir de la \textbf{factorización de Cholesky}, comenzando por computar la matriz de coeficientes $J^TJ$ y el lado derecho $J^Ty$. Después se computa la factorización de Cholesky
\begin{equation}
	J^TJ = \bar R^T\bar R.
\end{equation}
...
Como segunda posibilidad, tenemos la \textbf{factorización QR}, que evita el problema de depender del cuadrado del condicionamiento de $J$.


%conocido como \textit{ecuaciones normales} de (\ref{eq:flin}).
