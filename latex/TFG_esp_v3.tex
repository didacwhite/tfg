...
\begin{equation}
	...
	\label{eq:lsp}
\end{equation}
donde $r_i : \mathbb{R}^{n} \rightarrow \mathbb{R}$ son funciones continuamente diferenciables, conocidas como residuos.
Se asume que $m \geq n$ para este problema, de hecho en la práctica lo más común es que $m \gg n$. 

Veamos las propiedades de este modelo concreto de optimización sin restricciones y cómo se pueden aprovechar para formular algoritmos eficientes y robustos. Para ello, primero formamos un vector residuo con los 
$r : \mathbb{R}^{n} \rightarrow \mathbb{R}^{m}$ con los distintos componentes $r_i$, es decir
\begin{equation}
	r(x) = (r_1(x), r_2(x), \ldots, r_m(x))^T.
\end{equation}
Con esta notación, (\ref{eq:lsp}) se puede reformular como
$f(x) = \frac{1}{2} \Vert r(x) \Vert_2^2$. Las derivadas de $f$ se pueden reescribir en forma matricial, como la matriz Jacobiana $J(x)$, que será la matriz de las derivadas parciales de los residuos,
\begin{equation}
	J(x) = 
	\begin{bmatrix}
		\nabla r_1(x)^T \\
	...
	\end{bmatrix},
\end{equation}
y por tanto el gradiente y la Hessiana de f se expresan como sigue:

\begin{align}
	\nabla f(x) &= \sum_{i=1}^m r_i(x) \nabla r_i(x) = J(x)^Tr(x) \label{eq:grad}\\
	\nabla^2 f(x) &= \sum_{i=1}^m \nabla r_i(x) \nabla r_i(x)^T + \sum_{i=1}^m r_i(x)\nabla^2r_i(x) \nonumber \\
	&= J(x)^TJ(x)+\sum_{i=1}^m r_i(x)\nabla^2r_i(x). \label{eq:hess}
\end{align}

Si nos fijamos, el primer término de la matriz Hessiana se obtiene casi directamente del cálculo del gradiente (\ref{eq:grad}), así que el coste se reduce al segundo término $\sum_{i=1}^m r_i(x)\nabla^2r_i(x)$.
